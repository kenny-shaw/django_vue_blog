<template>
  <!-- 用户文章列表总区域 -->
  <el-container class="user-like-articles-list-container">
        ...
        <ul>
          <li>
            <a
              ...
              @click.prevent="toNewestUserLikeArticleList"
              >最新</a
            >
          </li>
          <li>
            <a
              href="#"
              :class="{ 'sort-active': !isNewest }"
              @click.prevent="toPopularUserLikeArticleList"
              >最热</a
            >
          </li>
        </ul>
    ...
    <el-main>
      <div
        class="user-like-articles-list-box"
        v-infinite-scroll="loadMoreUserLikeArticle"
        infinite-scroll-disabled="disabled"
      >
        <div
          class="user-like-article-box"
          v-for="article in userLikeArticleList"
          :key="article.id"
        >
          <div class="user-like-article-info-box">
            <!-- 存放作者、时间、栏目标签等信息 -->
            <ul class="user-like-article-meta-list">
              <li class="user-like-article-username">
                <a href="#" @click.prevent="toUserCenter(article.author_id)">{{
                  article.author
                }}</a>
              </li>
              <li class="user-like-article-created">
                <a href="#">{{ article.created }}</a>
              </li>
              <li class="user-like-article-column">
                <a href="#">{{ article.column }}</a>
              </li>
            </ul>
            <div class="user-like-article-tag-box">
              <el-tag
                size="mini"
                class="user-like-article-tag"
            ...
            <a
              href="#"
              class="user-like-article-title"
              @click.prevent="toArticleDetail(article.id)"
              >{{ article.title }}</a
            >
            <!-- 文章动作列表，包括点赞、评论等等 -->
            <ul class="user-like-article-action-list">
              <!-- 点赞 -->
              <li
          ...
          <!-- 文章标题图 -->
          <div
            class="user-like-article-avatar-box hidden-xxs-only "
            v-if="article.avatar"
          >
            <el-avatar
              shape="square"
...
export default {
  ...
  methods: {
    // 获取用户文章列表
    async getUserLikeArticleList(size = this.size, page = 1) {
      // 获取当前token
      const token = this.$store.state.token
      // 要请求的后端URL
      let backendUrl = `likearticles/${this.userid}?token=${token}`
      // 获取排序方式
      ...
      if (res.code === 1000) {
        ...
        if (this.userLikeArticleList.length === 0 || size === 6) {
          this.userLikeArticleList = res.data

          this.userLikeArticleList.forEach(article => {
            // 将时间转换为formnow
          ...
          this.userLikeArticleList = this.userLikeArticleList.concat(concatData)
        }
      } else if (res.code === 1001) {
        // 不存在数据时，将数组置位空
        this.userLikeArticleList = []
    ...
    // 加载用户的更多文章
    loadMoreUserLikeArticle() {
      console.log('loadmore...')
      this.loading = true
      // 首次加载时，会进行loadmore，判断userLikeArticleList是否为空
      // 如果为空进行首次加载，加载6条
      if (this.userLikeArticleList.length === 0) {
        console.log('获取6条')
        this.getUserLikeArticleList()
      } else {
        this.getUserLikeArticleList(this.loadMoreSize, this.loadMorePage)
        this.loadMorePage += 1
      }
      this.loading = false
    },
    // 访问用户最新文章路由
    toNewestUserLikeArticleList() {
    ...
    async LikeOrCancelLikeArticle(articleId) {
      const article = this.userLikeArticleList.find(
...
.user-like-articles-list-container {
  // background: yellow;
  height: 100%;
  width: 100%;
  // 头部区域
  ...
  .el-main {
    ...
    padding: 0;
    height: 100%;
    .user-like-articles-list-box {
      height: 100%;
      overflow: auto;
      .user-like-article-box {
        min-height: 115px;
        padding: 20px 30px;
        box-sizing: border-box;
        ...
        align-items: center;
        .user-like-article-info-box {
          max-width: 568px;
          flex: 1;

          ul.user-like-article-meta-list {
            a {
              font-size: 12px;
            }
            margin: 0;
            ...
            display: flex;
            align-items: center;
            li.user-like-article-username,
            li.user-like-article-created {
              position: relative;
              // 小圆点
              &::after {
                content: '·';
          ...
          .user-like-article-tag-box {
            width: 100%;
            display: flex;
            .user-like-article-tag {
              cursor: pointer;
              margin: 5px 5px 0 0;
            }
            // justify-content: space-around;
          }
          a.user-like-article-title {
            font-size: 17px;
            font-weight: 600;
            color: #2e3135;
            line-height: 2;
          ...
          ul.user-like-article-action-list {
            margin: 0;
            padding: 0;
            list-style: none;
            display: flex;
